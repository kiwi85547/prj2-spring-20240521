...
public interface BoardMapper {
    ...
    @Select("""
            SELECT b.id, b.title, b.content, m.nick_name writer, b.member_id, b.inserted
            FROM board b join member m ON b.member_id=m.id WHERE b.id=#{id}
                    """)
    ...
    @Select("""
                 <script>
                 SELECT b.id, b.title, m.nick_name writer, COUNT(DISTINCT f.name) number_of_images, 
            COUNT(DISTINCT l.member_id) number_of_like 
                 FROM board b JOIN member m ON b.member_id = m.id LEFT JOIN board_file f ON b.id=f.board_id LEFT JOIN board_like l ON b.id=l.board_id
    ...
    @Select("""
            ...
            OR m.nick_name LIKE #{pattern}
                    ...
                    """)
    Integer countAllWithPaging(String searchType, String keyword);
